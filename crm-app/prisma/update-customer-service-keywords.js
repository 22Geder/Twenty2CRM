const { PrismaClient } = require('@prisma/client')
const prisma = new PrismaClient()

/**
 * 🎯 סקריפט לעדכון מילות מפתח לשירות לקוחות
 * מוסיף את כל התגיות והמילות מפתח שיש במשרות טלרים גם למשרות שירות לקוחות
 */

// מילות מפתח בסיסיות לשירות לקוחות - זהות לטלרים
const CUSTOMER_SERVICE_KEYWORDS = [
  // כותרות עבודה
  'נציג שירות', 'נציגת שירות', 'נציג מוקד', 'נציגת מוקד', 
  'מוקדן', 'מוקדנית', 'שירות לקוחות', 'נציג טלפוני', 'נציגת טלפונית',
  
  // כישורים - בדיוק כמו טלרים!
  'שירותיות', 'סבלנות', 'יכולת הקשבה', 'פתרון בעיות',
  'תקשורת בינאישית', 'הקלדה מהירה', 'עבודה מול מחשב',
  'עבודה תחת לחץ', 'עמידה ביעדים', 'שימור לקוחות',
  'מערכת CRM', 'מערכת טלפוניה', 'משמרות',
  
  // ספציפי לבנקאות (כמו טלרים)
  'בנקאות', 'פיננסים', 'קופה', 'עסקאות', 'לקוחות פרטיים', 'לקוחות עסקיים',
  'קופאי', 'קופאית', 'טלר', 'טלרית',
  
  // מוקדים טלפוניים
  'מוקד מכירות', 'מוקד תמיכה', 'מוקד שירות', 'קול סנטר', 'Call Center',
  'טלמרקטינג', 'טלמרקטר', 'טלמרקטרית',
  
  // תעשייה
  'מכירות', 'מכירות טלפוניות', 'לידים', 'לידים חמים',
  'סגירת עסקאות', 'יעדים', 'עמלות', 'בונוס',
  
  // מילים אנגליות נפוצות
  'customer service', 'call center', 'helpdesk', 'support',
  
  // כישורים רכים
  'יחסי אנוש', 'תודעת שירות', 'אמינות', 'סדר וארגון',
  
  // 🆕 שפות - חיוני לשירות לקוחות!
  'עברית', 'עברית רמה גבוהה', 'עברית שפת אם',
  'ערבית', 'ערבית שפת אם', 'ערבית רמה גבוהה', 'דובר ערבית', 'דוברת ערבית',
  'אנגלית', 'אנגלית רמה גבוהה', 'דובר אנגלית', 'דוברת אנגלית',
  'רוסית', 'רוסית שפת אם', 'דובר רוסית', 'דוברת רוסית',
  'אמהרית', 'צרפתית', 'ספרדית',
  
  // 🆕 ניסיון קודם רלוונטי
  'ניסיון בקופה', 'ניסיון במכירות', 'ניסיון בשירות', 'ניסיון כקופאית',
  'עבודה בסופר', 'עבודה בחנות', 'עבודה מול לקוחות', 'קבלת קהל',
  'עבודה בקמעונאות', 'ייצור', 'עבודת יצור', 'עובדת יצור',
  'אורזת', 'אריזה', 'בית אריזה',
  
  // 🆕 מיקום - נגב ודרום
  'באר שבע', 'נשר', 'אשדוד', 'אשקלון', 'קריית גת',
  'רהט', 'לקייה', 'ערד', 'דימונה', 'אופקים', 'נתיבות', 'שדרות',
  'נגב', 'דרום', 'צפון', 'מרכז', 'גוש דן', 'שרון', 'חיפה',
  'להבים', 'עומר', 'מיתר',
  
  // 🆕 מחשבים ותוכנות
  'Office', 'אופיס', 'אקסל', 'Excel', 'וורד', 'Word', 
  'מחשב', 'שליטה במחשב', 'עבודה על מחשב', 'אוריינות מחשב',
  
  // 🆕 כישורים רכים נוספים
  'אחראית', 'אחראי', 'יצירתית', 'יצירתי', 'יוזמה', 'בעלת יוזמה',
  'כושר ביטוי', 'יכולת ביטוי', 'תקשורת', 'התמודדות עם לחץ',
  'השתלבות', 'עבודת צוות', 'למידה מהירה',
  
  // 🆕 רקע והשכלה
  'בגרות', 'תעודת בגרות', '12 שנות לימוד', 'תואר', 'קורס',
  'הנהלת חשבונות', 'מזכירות', 'מנהלה',
  
  // 🆕 סוגי תעסוקה
  'עבודה מהבית', 'היברידי', 'עבודה היברידית', 'משרה מלאה', 'משרה חלקית',
  'משמרות', 'זמינות מיידית', 'ללא ניסיון', 'מתאים לחסרי ניסיון',
  'entry level', 'התחלתית', 'ללא רקע קודם'
]

// מילות מפתח ספציפיות ל-YES - מכירות + שירות
const YES_KEYWORDS = [
  ...CUSTOMER_SERVICE_KEYWORDS,
  // ספציפי ל-YES
  'yes', 'יס', 'טלסל', 'סטינג', 'נטפליקס', 'דיסני',
  'אינטרנט', 'לוויין', 'כבלים', 'טלוויזיה', 'תקשורת',
  
  // מכירות
  'מכירות', 'נציג מכירות', 'נציגת מכירות', 'מוקד מכירות',
  'שיחות יוצאות', 'שיחות נכנסות', 'לידים חמים',
  'סגירת עסקאות', 'בונוסים', 'יעדים', 'טלמרקטינג',
  'הישגיות', 'תחרותיות', 'שכנוע', 'כושר ביטוי',
  
  // מיקום
  'נשר', 'באר שבע', 'צפון', 'דרום'
]

// תגיות לשירות לקוחות - מורחב עם שפות ומיקומים
const CUSTOMER_SERVICE_TAGS = [
  // בסיסיות
  'שירות לקוחות',
  'מוקד',
  'טלמרקטינג',
  'משרה מלאה',
  'לידים',
  'עמלות',
  
  // 🆕 שפות - חשוב מאוד לשירות לקוחות!
  'עברית',
  'ערבית',
  'אנגלית',
  'רוסית',
  
  // 🆕 רקע רלוונטי
  'קופאית',
  'מכירות',
  'ללא ניסיון',
  'entry level',
  
  // 🆕 מיקומים
  'באר שבע',
  'נשר',
  'דרום',
  'צפון',
  'נגב',
  
  // 🆕 כישורים
  'Office',
  'מחשב',
  'תקשורת'
]

// צבעי תגיות - מורחב
const TAG_COLORS = {
  'שירות לקוחות': '#06B6D4',    // ציאן
  'מוקד': '#3B82F6',             // כחול
  'טלמרקטינג': '#F97316',        // כתום
  'משרה מלאה': '#14B8A6',        // טיל
  'לידים': '#F59E0B',            // צהוב-כתום
  'עמלות': '#8B5CF6',            // סגול
  'מכירות': '#EF4444',           // אדום
  'yes': '#EC4899',              // ורוד
  'נשר': '#8B5CF6',
  'צפון': '#10B981',             // ירוק
  // 🆕 שפות
  'עברית': '#3B82F6',            // כחול
  'ערבית': '#10B981',            // ירוק
  'אנגלית': '#6366F1',           // אינדיגו
  'רוסית': '#EF4444',            // אדום
  // 🆕 מיקומים
  'באר שבע': '#F59E0B',          // כתום
  'דרום': '#F97316',             // כתום כהה
  'נגב': '#D97706',              // אמבר
  // 🆕 רקע
  'קופאית': '#EC4899',           // ורוד
  'ללא ניסיון': '#14B8A6',       // טיל
  'entry level': '#14B8A6',      // טיל
  // 🆕 כישורים
  'Office': '#6366F1',           // אינדיגו
  'מחשב': '#8B5CF6',             // סגול
  'תקשורת': '#06B6D4'            // ציאן
}

/**
 * בונה AI Profile לשירות לקוחות - זהה לטלרים
 */
function buildCustomerServiceAIProfile(position, region = '') {
  return {
    role: position.title,
    industry: 'שירות לקוחות ומוקדים',
    location: position.location,
    region: region,
    experience: 'ניסיון בשירות לקוחות או מכירות - יתרון משמעותי, מתאים גם ללא ניסיון',
    skills: [
      'שירות לקוחות',
      'תקשורת בינאישית',
      'עבודה בסביבה ממוחשבת',
      'סבלנות',
      'יכולת הקשבה',
      'פתרון בעיות',
      'מולטי-טאסקינג',
      'עבודה תחת לחץ'
    ],
    requirements: [
      'תודעת שירות גבוהה',
      'יכולת עבודה בסביבה ממוחשבת',
      'יחסי אנוש מעולים',
      'סבלנות והקשבה'
    ],
    keywords: CUSTOMER_SERVICE_KEYWORDS,
    idealCandidate: 'מועמד/ת עם תודעת שירות גבוהה, סבלנות ויכולת הקשבה, ניסיון בשירות לקוחות או מכירות - יתרון, יכולת עבודה בסביבה ממוחשבת',
    employmentType: 'משרה מלאה',
    workSchedule: 'משמרות',
    deepAnalysis: {
      basicInfo: {
        title: position.title,
        employer: position.employer?.name || '',
        location: position.location,
        region: region,
        department: 'שירות לקוחות'
      },
      requirements: {
        mustHave: [
          'תודעת שירות גבוהה',
          'יכולת עבודה מול מחשב'
        ],
        niceToHave: [
          'ניסיון בשירות לקוחות',
          'ניסיון במוקד'
        ],
        experience: {
          minimum: 0,
          preferred: 1,
          description: 'ניסיון בשירות - יתרון משמעותי'
        }
      },
      skills: {
        technical: ['עבודה בסביבה ממוחשבת', 'CRM'],
        professional: ['שירות לקוחות', 'פתרון בעיות', 'תקשורת'],
        soft: ['סבלנות', 'יכולת הקשבה', 'אמפתיה', 'יחסי אנוש'],
        languages: ['עברית']
      },
      industry: ['שירות לקוחות', 'מוקדים'],
      seniorityRequired: 'entry',
      keyRequirements: [
        'תודעת שירות',
        'עבודה מול מחשב',
        'יחסי אנוש'
      ]
    },
    lastAnalyzed: new Date().toISOString()
  }
}

/**
 * בונה AI Profile ל-YES - מכירות + שירות
 */
function buildYESAIProfile(position) {
  return {
    role: position.title,
    industry: 'טלקומוניקציה ותקשורת',
    location: position.location,
    region: position.location?.includes('נשר') ? 'צפון' : (position.location?.includes('באר שבע') ? 'דרום' : 'מרכז'),
    experience: 'ניסיון במכירות או שירות לקוחות - יתרון משמעותי, מתאים גם ללא ניסיון',
    skills: [
      'מכירות טלפוניות',
      'טלמרקטינג',
      'שירות לקוחות',
      'עבודה עם לידים',
      'סגירת עסקאות',
      'עבודה במוקד',
      'תקשורת בינאישית',
      'יכולת שכנוע',
      'עמידה ביעדים'
    ],
    requirements: [
      'אוריינטציה מכירתית',
      'יכולת עבודה בסביבה ממוחשבת',
      'כושר ביטוי גבוה',
      'יכולת שכנוע',
      'עבודה תחת לחץ',
      'הישגיות ותחרותיות'
    ],
    keywords: YES_KEYWORDS,
    idealCandidate: 'מועמד/ת עם אוריינטציה מכירתית, כושר ביטוי ושכנוע, ניסיון במכירות או שירות לקוחות, יכולת עבודה בסביבה ממוחשבת, הישגיות ותחרותיות. מתאים גם לחסרי ניסיון עם מוטיבציה גבוהה!',
    employmentType: 'משרה מלאה',
    workSchedule: 'משמרות',
    deepAnalysis: {
      basicInfo: {
        title: position.title,
        employer: 'YES',
        location: position.location,
        region: position.location?.includes('נשר') ? 'צפון' : 'מרכז',
        department: 'מכירות'
      },
      requirements: {
        mustHave: [
          'אוריינטציה מכירתית',
          'יכולת עבודה בסביבה ממוחשבת',
          'כושר ביטוי ויכולת שכנוע'
        ],
        niceToHave: [
          'ניסיון במוקדי שירות/שימור/מכירה',
          'ניסיון במכירות טלפוניות'
        ],
        experience: {
          minimum: 0,
          preferred: 1,
          description: 'ניסיון במכירות או שירות - יתרון משמעותי'
        }
      },
      skills: {
        technical: ['עבודה בסביבה ממוחשבת', 'CRM'],
        professional: ['מכירות', 'טלמרקטינג', 'סגירת עסקאות', 'ניהול לידים', 'שירות לקוחות'],
        soft: ['כושר ביטוי', 'יכולת שכנוע', 'אסרטיביות', 'תחרותיות', 'הישגיות', 'סבלנות'],
        languages: ['עברית']
      },
      industry: ['טלקומוניקציה', 'תקשורת', 'מכירות', 'שירות לקוחות'],
      seniorityRequired: 'entry',
      keyRequirements: [
        'מכירות טלפוניות',
        'עבודה עם לידים',
        'עמידה ביעדים',
        'שירות לקוחות'
      ]
    },
    lastAnalyzed: new Date().toISOString()
  }
}

async function main() {
  console.log('🚀 מתחיל עדכון מילות מפתח לשירות לקוחות...\n')

  // 1. יצירת כל התגיות הנדרשות
  console.log('📝 יוצר תגיות...')
  const allTags = [...new Set([...CUSTOMER_SERVICE_TAGS, 'מכירות', 'yes', 'נשר', 'צפון', 'באר שבע', 'דרום'])]
  
  for (const tagName of allTags) {
    await prisma.tag.upsert({
      where: { name: tagName },
      update: { color: TAG_COLORS[tagName] || '#6B7280' },
      create: { name: tagName, color: TAG_COLORS[tagName] || '#6B7280' }
    })
  }
  console.log(`✅ ${allTags.length} תגיות מוכנות\n`)

  // 2. עדכון משרות YES
  console.log('📌 מעדכן משרות YES...')
  const yesEmployer = await prisma.employer.findFirst({
    where: { name: { contains: 'YES', mode: 'insensitive' } }
  })

  if (yesEmployer) {
    const yesPositions = await prisma.position.findMany({
      where: { employerId: yesEmployer.id, active: true },
      include: { employer: true }
    })

    for (const position of yesPositions) {
      const region = position.location?.includes('נשר') ? 'צפון' : 
                     position.location?.includes('באר שבע') ? 'דרום' : 'מרכז'
      
      // תגיות למשרה
      const yesTags = ['מכירות', 'שירות לקוחות', 'מוקד', 'טלמרקטינג', 'לידים', 'עמלות', 'yes', 'משרה מלאה']
      if (region === 'צפון') yesTags.push('נשר', 'צפון')
      if (region === 'דרום') yesTags.push('באר שבע', 'דרום')

      await prisma.position.update({
        where: { id: position.id },
        data: {
          keywords: JSON.stringify(YES_KEYWORDS),
          aiProfile: JSON.stringify(buildYESAIProfile(position)),
          tags: {
            set: [],
            connect: yesTags.map(name => ({ name }))
          }
        }
      })
      console.log(`  ✅ ${position.title} - עודכן`)
    }
    console.log(`✅ עודכנו ${yesPositions.length} משרות YES\n`)
  } else {
    console.log('⚠️ לא נמצא מעסיק YES\n')
  }

  // 3. עדכון משרות שירות לקוחות כלליות (סלע לוגיסטיקה ואחרים)
  console.log('📌 מעדכן משרות שירות לקוחות כלליות...')
  const customerServicePositions = await prisma.position.findMany({
    where: {
      active: true,
      OR: [
        { title: { contains: 'שירות', mode: 'insensitive' } },
        { title: { contains: 'נציג', mode: 'insensitive' } },
        { title: { contains: 'מוקד', mode: 'insensitive' } },
        { title: { contains: 'לקוחות', mode: 'insensitive' } },
        { description: { contains: 'שירות לקוחות', mode: 'insensitive' } }
      ],
      // לא כולל בנקאים וטלרים (כבר מעודכנים)
      NOT: [
        { title: { contains: 'טלר', mode: 'insensitive' } },
        { title: { contains: 'בנקאי', mode: 'insensitive' } },
        { employer: { name: { contains: 'מזרחי', mode: 'insensitive' } } }
      ]
    },
    include: { employer: true }
  })

  let updated = 0
  for (const position of customerServicePositions) {
    // בדיקה אם זו משרת yes - כבר טופל
    if (position.employer?.name?.toLowerCase().includes('yes')) continue

    // תגיות כלליות לשירות לקוחות
    const tags = ['שירות לקוחות', 'מוקד', 'משרה מלאה']
    if (position.title?.includes('מכירות') || position.description?.includes('מכירות')) {
      tags.push('מכירות', 'עמלות')
    }

    await prisma.position.update({
      where: { id: position.id },
      data: {
        keywords: JSON.stringify(CUSTOMER_SERVICE_KEYWORDS),
        aiProfile: JSON.stringify(buildCustomerServiceAIProfile(position)),
        tags: {
          connect: tags.map(name => ({ name }))
        }
      }
    })
    console.log(`  ✅ ${position.title} (${position.employer?.name}) - עודכן`)
    updated++
  }
  console.log(`✅ עודכנו ${updated} משרות שירות לקוחות\n`)

  // 4. סיכום
  const allPositions = await prisma.position.count({ where: { active: true } })
  console.log('========================================')
  console.log('✨ העדכון הושלם!')
  console.log(`📋 סה"כ משרות פעילות: ${allPositions}`)
  console.log('========================================')
}

main()
  .catch((e) => {
    console.error('❌ שגיאה:', e)
    process.exit(1)
  })
  .finally(async () => {
    await prisma.$disconnect()
  })
