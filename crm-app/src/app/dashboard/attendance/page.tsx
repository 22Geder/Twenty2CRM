...
export default function AttendancePage() {
  ...
  // ⏱️ חישוב זמן חי בעבודה (מתעדכן כל שנייה דרך state של now)
  const todayBreakMinutes = todayRow?.breakMinutes ?? 30
  const liveElapsedMs = isClockedIn && todayRow?.clockIn
    ? Math.max(0, now.getTime() - new Date(todayRow.clockIn).getTime())
    : 0
  const liveElapsedSec = Math.floor(liveElapsedMs / 1000)
  const liveGrossMin = Math.floor(liveElapsedSec / 60)
  // נטו בזמן אמת: מורידים הפסקה רק כשעבדת כבר מעל ההפסקה
  const liveNetMin = Math.max(0, liveGrossMin - todayBreakMinutes)
  ...
  // 📊 סיכום היום (סופי - אחרי יציאה)
  const finalWorkedMin = isClockedOutToday && todayRow
    ? calcWorkedMinutes(todayRow.clockIn, todayRow.clockOut, todayRow.breakMinutes)
    : 0
  const finalGrossMin = isClockedOutToday && todayRow?.clockIn && todayRow?.clockOut
    ? Math.max(0, Math.floor((new Date(todayRow.clockOut).getTime() - new Date(todayRow.clockIn).getTime()) / 60000))
    : 0
            ...
            <div className="mt-4 rounded-xl border-2 border-green-300 bg-gradient-to-br from-green-50 to-emerald-50 p-4">
              <div className="flex flex-col md:flex-row items-center justify-between gap-3">
                <div className="text-center md:text-right">
                  ...
                  <div className="font-mono text-3xl md:text-4xl font-bold text-green-800 mt-1">
                    {formatHMS(liveElapsedSec)}
                  </div>
                  <div className="text-xs text-green-600 mt-1">
                    מאז {formatTime(todayRow?.clockIn ?? null)}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-white/70 rounded-lg px-3 py-2 border border-green-200">
                    <div className="text-[10px] text-gray-500 uppercase">ברוטו</div>
                    <div className="font-mono font-bold text-gray-800">{formatMinutes(liveGrossMin)}</div>
                  </div>
                  <div className="bg-white/70 rounded-lg px-3 py-2 border border-amber-200">
                    <div className="text-[10px] text-gray-500 uppercase flex items-center justify-center gap-1">
                      <Coffee className="w-3 h-3" /> הפסקה
                    </div>
                    <div className="font-mono font-bold text-amber-700">{todayBreakMinutes}׳</div>
                  </div>
                  <div className="bg-white/90 rounded-lg px-3 py-2 border-2 border-green-400">
                    <div className="text-[10px] text-green-700 uppercase font-bold">נטו</div>
                    <div className="font-mono font-bold text-green-800">{formatMinutes(liveNetMin)}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl border-2 border-blue-300 bg-gradient-to-br from-blue-50 to-indigo-50 p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <div className="bg-white rounded-lg px-3 py-2 border border-blue-200 text-center">
                  <div className="text-[10px] text-gray-500 uppercase flex items-center justify-center gap-1">
                    <LogIn className="w-3 h-3" /> כניסה
                  </div>
                  <div className="font-mono font-bold text-gray-800">{formatTime(todayRow.clockIn)}</div>
                </div>
                <div className="bg-white rounded-lg px-3 py-2 border border-blue-200 text-center">
                  <div className="text-[10px] text-gray-500 uppercase flex items-center justify-center gap-1">
                    <LogOut className="w-3 h-3" /> יציאה
                  </div>
                  <div className="font-mono font-bold text-gray-800">{formatTime(todayRow.clockOut)}</div>
                </div>
                <div className="bg-white rounded-lg px-3 py-2 border border-blue-200 text-center">
                  <div className="text-[10px] text-gray-500 uppercase">ברוטו</div>
                  <div className="font-mono font-bold text-gray-800">{formatMinutes(finalGrossMin)}</div>
                </div>
                <div className="bg-white rounded-lg px-3 py-2 border border-amber-200 text-center">
                  <div className="text-[10px] text-gray-500 uppercase flex items-center justify-center gap-1">
                    <Coffee className="w-3 h-3" /> הפסקה
                  </div>
                  <div className="font-mono font-bold text-amber-700">{todayRow.breakMinutes}׳</div>
                </div>
                <div className="col-span-2 md:col-span-1 bg-green-100 rounded-lg px-3 py-2 border-2 border-green-400 text-center">
                  <div className="text-[10px] text-green-700 uppercase font-bold">נטו</div>
                  <div className="font-mono font-bold text-green-800 text-lg">{formatMinutes(finalWorkedMin)}</div>
                </div>
              </div>
            </div>
          )}
