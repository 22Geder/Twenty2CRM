...
export async function POST(request: Request) {
  try {
    ...
    const servicePosition = await prisma.position.upsert({
      where: {
        employerId_title: {
          employerId: employer.id,
          title: 'נציג/ת שירות לקוחות - מוקד ארצי'
        }
      },
      update: {
...
📍 מועמדים מהקריות וחיפה - בעדיפות!`,
        ...
        employerId: employer.id,
        tags: {
          connect: serviceTags.map(t => ({ id: t.id }))
        }
      }
    })
    ...
    const salesGalilot = await prisma.position.upsert({
      where: {
        employerId_title: {
          employerId: employer.id,
          title: 'נציג/ת מכירות יד ראשונה - סניף גלילות'
        }
      },
      update: {
        description: `🚗 מחפשים נציגי/ות מכירות מצוינים וחרוצים להובלת תהליכי מכירה מלאים!
...
• נכונות לעבודה 6 ימים בשבוע`,
        ...
        salaryRange: '14,000-17,000 ₪ + עמלות גבוהות (אין תקרה!)',
        active: true,
        tags: {
          set: salesTags.map(t => ({ id: t.id }))
        }
      },
      create: {
        title: 'נציג/ת מכירות יד ראשונה - סניף גלילות',
...
• נכונות לעבודה 6 ימים בשבוע`,
        ...
        employerId: employer.id,
        tags: {
          connect: salesTags.map(t => ({ id: t.id }))
        }
      }
    })
    ...
    // 3. משרת מכירות - אשדוד
    const salesAshdod = await prisma.position.upsert({
      where: {
        employerId_title: {
          employerId: employer.id,
          title: 'נציג/ת מכירות יד ראשונה - סניף אשדוד'
        }
      },
      update: {
        description: `🚗 מחפשים נציגי/ות מכירות מצוינים וחרוצים להובלת תהליכי מכירה מלאים!
...
• נכונות לעבודה 6 ימים בשבוע`,
    ...
    const salesBeerSheva = await prisma.position.upsert({
      where: {
        employerId_title: {
          employerId: employer.id,
          title: 'נציג/ת מכירות יד ראשונה - סניף באר שבע'
        }
      },
      update: {
        description: `🚗 מחפשים נציגי/ות מכירות מצוינים וחרוצים להובלת תהליכי מכירה מלאים!
